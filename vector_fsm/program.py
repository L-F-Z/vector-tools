...
class StateMachineProgram(StateNode):
    ...
    def stop(self):
        super().stop()
        try:
            self.robot.world.remove_event_handler(vector.world.EvtNewCameraImage,
                                                  self.process_image)
        except: pass
    ...
    def process_image(self,event,**kwargs):
        curim = numpy.array(event.image.raw_image) #vector-raw image
        ...
        if self.force_annotation or self.windowName is not None:
            ...
            # Apply Vector SDK annotations and rescale.
