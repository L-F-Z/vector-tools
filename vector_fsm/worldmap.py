...
class RobotForeignObj(WorldObject):
    def __init__(self, vector_id=None, x=0, y=0, z=0, theta=0, camera_id = -1 ):
        super().__init__(id,x,y,z)
    ...
    def __repr__(self):
        return '<RobotForeignObj %d: (%.1f, %.1f, %.1f) @ %f.> from camera %f\n' % \
               (self.vector_id, self.x, self.y, self.z, self.theta*180/pi, self.camera_id)
...
class LightCubeForeignObj(WorldObject):
    light_cube_size = (44., 44., 44.)
    def __init__(self, id=None, vector_id=None, x=0, y=0, z=0, theta=0, is_visible=False):
        super().__init__(id,x,y,z)
        self.theta = theta
        self.vector_id = vector_id
        self.size = self.light_cube_size
    ...
    def __repr__(self):
        return '<LightCubeForeignObj %d: (%.1f, %.1f, %.1f) @ %d deg.> by vector %d \n' % \
               (self.id, self.x, self.y, self.z, self.theta*180/pi, self.vector_id)
...
class WorldMap():
    vision_z_fudge = 10  # Vector underestimates object z coord by about this much
