...
class Event:
    ...
    vector_evt_type = None

    def generator(self,erouter,vector_evt): pass
...
class EventRouter:
    ...
    def add_listener(self, listener, event_class, source):
        ...
        if source_dict is None:
            source_dict = dict()
            # start a vector event handler if this event type requires one
            if event_class.vector_evt_type:
                coztype = event_class.vector_evt_type
                if not issubclass(coztype, vector.event.Event):
                    raise ValueError('%s vector_evt_type %s not a subclass of vector.event.Event' % (event_type, coztype))
    ...
    def remove_listener(self, listener, event_class, source):
        ...
        if len(source_dict) == 0:   # no one listening for this event
            ...
            # remove the Vector SDK event handler if there was one
            if event_class.vector_evt_type:
                coztype = event_class.vector_evt_type
                world = self.robot.world
